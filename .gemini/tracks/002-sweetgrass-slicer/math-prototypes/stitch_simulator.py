...
class StitchSimulator:
    ...
    def calculate_stitches(self, path, pitch=5.0):
        ...
        points_per_rev = len(path) // (int(max(path[:,2]) / pitch) if max(path[:,2]) > 0 else 1)
        
        for i in range(points_per_rev, len(path)):
            ...
            if i % (points_per_rev // self.stitches_per_rev) == 0:
                ...
                # In a perfect spiral, this is i - points_per_rev
                anchor_pt = path[max(0, i - points_per_rev)]
